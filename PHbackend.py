...
try:
    ...
    if platform.system().lower().startswith('win'):
        os.system("pip install delegator")
    else:
        os.system("pip3 install delegator")
    import delegator
...
try:
    ...
    if platform.system().lower().startswith('win'):
        ...
        os.system("pip3 install crayons")
    import crayons
...
class Video:
    ...
    def download_480p(self, quality: str = None, block=True):
        print(crayons.red('Downloading video…'))
        c = delegator.run('youtube-dl -f 135 {}'.format(self.url), block=block)
        return c
    
    def download_360p(self, quality: str = None, block=True):
        print(crayons.red('Downloading video…'))
        c = delegator.run('youtube-dl -f 134 {}'.format(self.url), block=block)
        return c
